...
//******************************Set Price function**********************************
    ...
    function setPricePeriod(uint256 _SetPrice,uint256 _DayValid, bytes32 _PIN) public pinCheck(_PIN) contractExpired
    {
        
    savePrevPricePeriode();
    
    
        if((_DayValid>0) && (_SetPrice>0)){
        
            priceEnergy=_SetPrice;
            declareDate=now;
            startDate=now.add( 15 minutes);
            expireDate=startDate.add(( _DayValid.mul( 1 minutes)));
        
        }
        else if((_DayValid==0) && (_SetPrice==0)){
