...
function sendSign(myData,gasLimit){
    web3.eth.getTransactionCount(current_account, (err, txCount) => {
        ...
        const transaction = web3.eth.sendSignedTransaction(raw)
            ...
            .then(receipt => {
                t1 = performance.now();
                console.log(`Blockchain execution time : ${t1 - t0} milliseconds.`);
...
function _updateMyUserData() {
    ...
    sendSign(myData,mTx);
    console.timeEnd("updateMyUserData");
}
...
function _onload(){
    _getMyUserdata();
    _getMyCapacity();
    document.getElementById("_addressUser").innerHTML=current_account;
